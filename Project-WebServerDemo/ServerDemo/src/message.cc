...
void mqtt_message_dump(mqtt_message_t* message) {
  ...
  cout << message->common.type << " " << MQTT_TYPE_PUBLISH << endl;
  if (message->common.type == MQTT_TYPE_PUBLISH) {
    ...
    printf("\n");
    printf("  message_id:        ");
    cout << message->publish.message_id;
    printf("\n");
  }
  if (message->common.type == MQTT_TYPE_SUBSCRIBE) {
    printf("  topic_name:        ");
    mqtt_buffer_dump(&(message->subscribe.topics->name));
    printf("\n");
    printf("  message_id:        ");
    cout << message->subscribe.message_id;
    printf("\n");
  }
}
