...
- ~~以下内容还无法达到很好的理解~~：
  ...
  在正常操作下，通过object调用virtual函数本身就是固定的，也就是不需要虚表多态那套东西，可以当成调用普通成员函数处理，对编译器来说，这是一种优化操作，不是必须实现的功能。
...
- 对于class作者已经设定有constructor的情况（不管是不是default constructor），编译器都不会再合成default constructor，而是将需要初始化的数据的初始化代码置于所有的constructor的user-code用户代码的上面，初始化顺序按照data member的声明顺序来。
...
- 在copy constructor中使用memset、memcpy的操作时，一定要注意避免修改vptr，因为vptr的初始化代码会由编译器合成置于user-code上面。

- 小知识：在严谨的C++用词中，“定义”是指“占用内存“的行为

- 将一个object作为一个函数的参数时，编译器可能会有两种策略：
  ...
  - 第二种，直接拷贝构建，对参数实际在函数栈中的位置直接用实参值拷贝构建（placement new？）
...
- 若对于所有的 `A f()` 都return一个具名数值，那么对于上一条，编译器可以做这样的优化，代码如下：
  ...
  (TODO::没搞懂为什么使用NRV优化还需要explicit copy constructor? 很奇怪，明明开启NRV优化后省去了一步copy constructor，explicit copy constructor是不是触发NRV的条件？)

- 使用member initialization list最大的好处就是能省去几步操作，至少不使用initialization list进行初始化操作而使用赋值操作的话，至少会多一个赋值操作的调用，可能还会产生一个临时对象之后再destructor。显然，使用member initialization list效率会更高。（还会避免一些奇怪赋值带来的奇怪问题？）
...
- vptr现在一般都是放在类的首端，以前都是放末端，放末端可以让类维持C strcut的访问方式。
  ...
  不过vptr放在首端有一个弊端，就是当父类没有虚函数不需要建立虚表，子类有虚函数需要建立虚表，就会有一定额外开销。（？？？编译器需要多加一些判断代码？？？base class pointer是怎么判断当前是否需要使用多态状态？默认应该是不用多态才对）

- 单一继承的时候，指针的类型转换很简单，指针都是指向同一个内存起始点，但是当多重继承时，由于有多个父类的缘故，指向子类的指针转换为指向父类的指针时，需要做一些偏移计算，会有更多消耗。（编译器可以优化，不过仔细想想，这一步的偏移计算无论如何还是需要的吧？所以相对单一继承来说，多重继承指针的转化（还有存取？）会慢一些）

- 对于使用虚拟继承（又或者使用虚函数，也就是使用C++的多态性质），使用者使用到它的时候自然是需要使用到它的多态性质（又或是在空间与效率的平衡上选择了空间更优，不过这和我接下来想说的并无关系）也就是说，对于使用者来说使用到虚拟继承的时候就是要使用它的多态性质了，因此对于编译器来说，一律按照多态方式解读代码并无太大不妥。原文如下：
...
- 在多重继承体系下，若存在虚函数，那么对于调用其成员函数就需要做出以下判断：
  ...
  - 函数是虚函数，通过索引信息间接调用虚表中的函数。此时就会产生一个问题，因为调用成员函数需要传递this指针，那此时需要传递给函数的this指针是哪个？编译器无法得知运行时的信息，那么就需要对象本身本身维护，编译器会为其产生一些额外信息用来修改传递的this指针。
...
- 建议在虚拟继承体系中，虚父类不含nonstatic data members，它会导致对成员以及函数的访问负担增加...可能还会出现难以调试的奇怪问题。（Effectiv C++中也有提到，TODO::有空的话可以研究一下。）
...
- 书中有说inline可能会导致大量临时变量的生成，按照我的理解，临时变量产生那应是要产生的，即使不inline也会产生，inline导致最大的问题就是，在同一个表达式中，产生的临时变量会同时存在，这就是个问题。（结合第6章临时变量部分，临时变量是在表达式结束后销毁的，因此inline会使临时变量大量堆积。）
...
- 理解表达式中每一步的转化操作有助于优化表达式的效率，有些表达式看起来很简短，实际上做了好多步的转化操作。

  简单优化就是将表达式所有的隐式转换写出来，这样能一定程度上加快编译效率。

- TODO::destructor代码是会被安插在所有return之前吗（言外之意就是return越多安插的代码就会等比膨胀吗）？不是很确定，感觉从后面的内容中（第7章）是想说会维护一个区域表的东西。

- global objects都会被放置在data segment中，显示赋值就会以此为初值，否则默认为0。

  若其有constructor和destructor，则需要静态初始化操作和内存释放操作。静态初始化操作也就是其无法在编译器进行初始化赋值，因此需要生成初始化所需要使用的代码，将其放入main函数的入口处，在main运行时进行初始化操作。

  一般所有的静态初始化操作和内存释放操作都会被整理在一个文件中，通过在main入口处加入__main()函数和exit()函数进行调用。

  静态初始化的一个问题是，若是初始化代码中存在throw操作，也就是exception，而当前的静态初始化代码也不会被放在try块中，也就是对程序员来说，丧失了对异常情况的控制权，这是一个很大的问题。因此..还是慎用拥有静态初始化的objects吧。（感觉确保没有throw也是个思路？）

- 支持“nonclass object的静态初始化”在某种程度上是支持virtual base class的一个副产品。

  虚继承体系中的指针在赋值时，若涉及virtual base class，在某些时候就需要在运行时侯才能成功赋值，在编译器无法判断指针的偏移量（运行期多态）。

  因此当它们的赋值操作是在global中进行时，同class object一样，需要将赋值或者初始化代码移入main的开头处，进行执行期判断。

- 局部静态变量的初始化和内存释放操作，是通过一组条件变量进行判定。意思差不多是第一次初始化时变量设为true，默认为false。

- 对象数组的初始化和内存释放是通过两个函数实现，vec_new和vec_delete，有virtual base class可能会调用另外的初始化函数vec_vnew。

  这里需要关注的是，只要数组元素中有一个初始化失败，就会将原来初始化完毕的对象都释放掉。

  因为vec_new是通过传递函数指针的方式调用constructor，若是constructor有默认的构造参数，那么通过指针调用就无法获得其依赖的默认参数，编译器可能的做法是构造出一个无参的构造函数（在必要时刻），在其中调用有参构造函数。形如：
...
- 小知识：new运算符分为两步操作：1、分配内存；2、内存分配成功，调用构造函数构造，若构造失败会释放已分配内存。

- 对数组进行delete时，delete所需信息来自于传入的指针，举例如下：

  ```c++
  class Point {...};
  class Point3D : public Point {...};
  Point *p = new Point3D[10];
  delete[] p;                       //此时的delete会产生错误，它在对指针偏移时，使用
  								  //的是指针p的类型信息，也就是sizeof(Point)，而
  								  //非sizeof(Point3D)。
  ```

- placement new operator操作可以对一块现有的内存做初始化构造操作。

  若该内存已经被构造过，placement new operator会将新的数据覆盖在原有数据之上，而非先调用析构函数。析构函数需要手动调用。

  若是在小内存中构造大object，那就会发生不可预期的问题。（操作了为申请的内存空间？）

  placement new operator可以操作出未定义的操作，但是并不能改变一些语言本身的性质，代码如下：
  ...
  b.f();                  // 调用的是Base的f()函数
  new (&b)Derived;
  b.f();                  // 调用的依然是Base的f()函数。（当然这并非一定）
  						// 按照我的理解，b这个object就是要使用Base去解析，
                          // new (&b)Derived可以理解为一个编译器不禁止的非法操作。
  ```

- 表达式中临时对象生成后，是在表达式结束时销毁，感觉简单理解就是分号前或者是一串连续语句的结束时。

  `if(表达式) { }`其中生成的临时变量会在右括前销毁。

  对临时变量进行引用，那就是在引用变量销毁时临时变量销毁或者是临时变量的生命范畴结束时销毁。（之前一直以为不能进行 `string &s = " ";` 这种操作，感觉有点儿像是右值引用？）

  注：临时变量会对程序产生负担，因此清楚临时变量得生成情况，并针对其优化，不仅是编译器需要做的，也是程序员需要去努力实现的。

### 第7章 站在对象模型的尖端

- 实例化模板类只有当需要用到它的时候才会实例化，比如定义一个类，又或者是对其中的静态变量赋初值。（还有很多隐式的使用需要当心）

  这里有一点要注意，定义一个指针本身并不会造成模板的实例化，但是若是对指针进行操作，就会造成模板的实例化，比如`Point<int> *p = 0; p = p + 1;`。（必要情况才实例化）

- 对于类的模板成员函数，只有当他们真正需要用到时才会被实例化（同上）。

- 模板中的错误检查，一般取决于编译器的设计者，也就是可能会有很多程序员一眼就能看出问题的代码，而编译器不报错。（当然若是实例化那肯定就会检查出来了）

- 对于模板中使用的外部名称，若使用的名称（变量或者函数名）与模板参数无关，那么这些名称对应的内容将会在模板定义的区域中寻找；而若是使用的名称与模板参数有关，那么就会在模板实例化的地方寻找这些名称所对应的内容。

- 由于exception handling需要编译器提供exception objects的相关信息（运行时），以知道其类型信息，这也就产生了RTTI这一副产品。

- 在catch语句中对exception object的改变都是局部性的。（除非是引用catch？）

- 小知识：使用dynamic_cast运算符可以在执行期决定真正的类型，判断类型转换是否安全（尤其是向下转换这种操作），若转换失败会返回0（若是引用类型的话，就会抛出bad_cast exception的异常），转换成功则返回转换后的指针。

- 小知识：typeid也可以在运行期获得对象类型信息，对于内置类型，则是静态获取，而非运行期获取，且是必要时才获取。

- 7.4节 .......大概知道想说明在某些领域的弹性不足，但由于缺乏对应的知识，看得云里雾里，没有很大感触.....
