...
- adapters简单理解就是（举个例子）：有一群不同职业的人，如果想让他们各自去赚钱，那么需要各自告诉他们去干自己职业的事，也就是对于赚钱这个指令，他们不知道要去干什么，比如医生，想让他们去赚钱，就得告诉他们去医院上班，给人看病。那么对于上层管理这些人的人来说，想让他们都去赚钱，就得根据他们不同的职业去发布指令。现在有这么个耳机，这个耳机可以识别戴上的人的职业，然后将赚钱指令转换成该职业需要听到的命令。当所有人都带上这个耳机后，管理者只要发布赚钱的命令就行了，而不需要去根据不同人的职业去发布命令。这里的这个耳机，就可以理解为配接器。

  配接器就是将下层对象纷繁散乱的能力组织起来，实现当前配接器的需求能力，并且转换成统一接口，供上层使用。（注：这里的上下层某种意义上是同级关系，可以理解为左右端）

- 有必要说明一下reverse_iterator（逆向迭代器），每一个逆向迭代器都有一个对应的正向迭代器，可以理解为逆向迭代器都是基于正向迭代器的。但是对于逆向迭代器，它所指向的对象是它所维护的正向迭代器-1后的元素，这一点很重要。

- 感觉这一章感觉和上一章很像，感觉内容有点儿虚？代码本身没有什么特别惊艳的地方，然后对adapters没有做一些深入的阐述和思考，adapters的特性没有很好的从代码中剥离出来...
